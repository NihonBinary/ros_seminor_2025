...
int main(int argc, char * argv[])
{
  ...
  // ロガーを生成
  auto const logger = rclcpp::get_logger("hello_moveit3");
  ...
  RCLCPP_INFO(logger, "Start Pose Position: x: %.2f, y: %.2f, z: %.2f", 
    start_pose.position.x, start_pose.position.y, start_pose.position.z);
  RCLCPP_INFO(logger, "Start Pose Orientation: x: %.2f, y: %.2f, z: %.2f, w: %.2f", 
  ...
  // 速度と加速度のスケーリングを設定
  double velocity_scaling_factor = 0.01;  // 速度スケーリング (0.0〜1.0)
  double acceleration_scaling_factor = 0.01;  // 加速度スケーリング (0.0〜1.0)
